	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(Company c : DataRepository.GetCompanies()){
			if(	c.getName().toLowerCase().contains(param.sSearch.toLowerCase())
				...
				c.getStartdate().toLowerCase().contains(param.sSearch.toLowerCase())
				||
				c.getEnddate().toLowerCase().contains(param.sSearch.toLowerCase()))
		...
		Collections.sort(companies, new Comparator<Company>(){
			...
			public int compare(Company c1, Company c2) {	
				...
				case 0:
					return c1.getTower().compareTo(c2.getTower()) * sortDirection;
				case 1:
					return c1.getName().compareTo(c2.getName()) * sortDirection;
				
				case 2:
					return c1.getEvent().compareTo(c2.getEvent()) * sortDirection;
				...
				case 5:
					return c1.getStartdate().compareTo(c2.getEnddate()) * sortDirection;	
		...
		try {
			...
			for(Company c : companies){
				...
				row.add(new JsonPrimitive(c.getTower()));
				row.add(new JsonPrimitive(c.getName()));
				row.add(new JsonPrimitive(c.getEvent()));
				
				row.add(new JsonPrimitive(c.getAddress()));
				row.add(new JsonPrimitive(c.getTown()));
				row.add(new JsonPrimitive(c.getStartdate()));
				row.add(new JsonPrimitive(c.getEnddate()));				
				data.add(row);
			}
